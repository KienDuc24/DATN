// public/game/ToD/ToDSocket.js (hoặc game_handlers/ToDSocket.js)
...
module.exports = (socket, io) => {
  ...
  socket.on('tod-who', async ({ roomCode }) => {
    try {
      ...
      io.to(roomCode).emit('tod-joined', payload); // Cập nhật cho mọi người
    } catch (e) { console.error('[ToD] tod-who error', e); }
  });

  // --- SỬA LỖI LOGIC 'tod-join' ---
  socket.on('tod-join', async ({ roomCode, player }) => {
    try {
      console.log('[ToD] tod-join received', { socketId: socket.id, roomCode, player });
      
      // 1. Tìm phòng. Nó *phải* tồn tại và đang "playing".
      const room = await Room.findOne({ code: roomCode, status: 'playing' }).lean(); // Dùng .lean()
      ...
      // 2. Kiểm tra xem người chơi này có trong danh sách không
      // (Danh sách này đã được chốt bởi phòng chờ)
      ...
      // 3. Cho socket vào phòng
      socket.join(roomCode);
      gameSocketMap.set(socket.id, { player: playerName, code: roomCode });
      getRoomState(roomCode); // Khởi tạo state (nếu chưa có)

      // 4. Lấy avatar (logic này của bạn đã đúng)
      ...
      const payload = {
        ...
        players: playersWithAvt, // Gửi danh sách 2 người
        createdAt: room.createdAt || null,
        updatedAt: room.updatedAt || null,
        state
      };
      
      // 6. Gửi lại cho mọi người
  ...
  socket.on('tod-start-round', async ({ roomCode }) => {
     try {
      ...
      // Không cần 'locked', vì status='playing' đã khóa rồi
  ...
  socket.on('tod-vote', async ({ roomCode, player, vote }) => {
     try {
      ...
      const playersNorm = getPlayersFromRoom(room);
      if (playersNorm.length <= 1) return; // Không vote nếu chỉ có 1 người
  ...
  // --- SỬA LỖI: LOGIC DISCONNECT VÀ XÓA PHÒNG ---
